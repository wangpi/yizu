...
//发问答
	...
	//换问题
	$('.Js-change-ques').on('click',function(){
		//点击此个，li隐藏，下一个li显示
		var lis=$("#ques-group li");
		$(this).parent("li").removeClass('curr');
		var indexnum=$(this).parent("li").index();
		var len=lis.length-1,newUrl;
		if(indexnum===len){
			//当点到最后一个时，显示第一个
			lis.eq(0).addClass('curr');
		}else{
			$(this).parent("li").next().addClass('curr');
		}
		newUrl=$('#ques-group .curr').find(".ques-title").attr("href");
		$("#js-to-answer").attr("href",newUrl);
	});
