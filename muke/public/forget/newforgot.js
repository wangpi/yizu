...
function(){
    ...
    var refreshVerifyCode=function(){
        var signUpForm = $("#js-forgot-form"),
            verifyCodeUrl = '/user/getcode?';
        signUpForm.find('.verify-img').attr('src', verifyCodeUrl + new Date().getTime());
    }

    refreshVerifyCode();

    $(".js-change-verify-code").on('click',function(){
        refreshVerifyCode();
    });
    /*$(".js-change-verify-code").on('click',function(){
        var $p=$(this).prev("img");
    ...
    $("#js-forgot-submit").button({loadingText:"正在提交..."})
    .on("click",function(){
        var $this=$(this);
                    ...
                    success:function(data){
                        ...
                        if(data.status===1){
                            ...
                            if(link=genericEmailLinksMap[vals.email.match(/[^@]*$/)[0]]){
                                $(".js-forgot-result").show().find(".js-email-add").append("<br /><a href='"+link+"' target='_blank'>查看邮件&gt;&gt;</a>");
                            }
                        }
                        forgotMsg(data.msg||"");
                        refreshVerifyCode();
